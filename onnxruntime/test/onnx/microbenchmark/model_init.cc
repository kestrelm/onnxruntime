...
Status CreateModelFromStr(const char* str, std::unique_ptr<Model>* out) {
  onnx::ModelProto mp;
  if (!google::protobuf::TextFormat::ParseFromString(str, &mp))
...
Status CreateExecutionProviders(std::unique_ptr<ExecutionProviders>* ret) {
...
#ifdef USE_CUDA
  {
    ...
    ORT_RETURN_IF_ERROR(execution_providers->Add(onnxruntime::kCudaExecutionProvider, std::make_unique<CUDAExecutionProvider>(epi)));
  }
#endif
#ifdef USE_MKLDNN
  {
    MKLDNNExecutionProviderInfo epi;
    ORT_RETURN_IF_ERROR(execution_providers->Add(onnxruntime::kMklDnnExecutionProvider, std::make_unique<MKLDNNExecutionProvider>(epi)));
  }
#endif
  {
    CPUExecutionProviderInfo epi;
    ORT_RETURN_IF_ERROR(execution_providers->Add(onnxruntime::kCpuExecutionProvider, std::make_unique<CPUExecutionProvider>(epi)));
  }
...
static void BM_PartitionModel_tiny_yolo(benchmark::State& state) {
  ...
  if (!model_proto.ParseFromCodedStream(coded_input.get()))
    throw std::runtime_error("open test data failed");
  std::unique_ptr<ExecutionProviders> execution_providers;
