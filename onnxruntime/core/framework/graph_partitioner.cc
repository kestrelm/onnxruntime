 ...
 */
static Node* PlaceNode(Graph& graph, std::unique_ptr<IndexedSubGraph> capability, const KernelRegistryManager& kernel_registry_mgr, const std::string& provider_type, int& count) {
...
Status GraphPartitioner::Partition(Graph& graph, bool export_dll, FuncManager& func_mgr) const {
  ...
  for (auto& provider : providers_) {
    ...
    std::vector<std::unique_ptr<ComputeCapability>> capabilities = provider->GetCapability(graph_viewer,
